...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        # If invalid, Django will attach errors and fall through to render below.
...
def manage_users(request):
    ...
    if (
        ...
        or (
            ...
            and request.user.profile.role != "ADMINISTRATOR"
        )
    ):
        return redirect("home.index")

    # Get all profiles to show in the table
    users = Profile.objects.select_related('user').all().order_by('role', 'user__username')
